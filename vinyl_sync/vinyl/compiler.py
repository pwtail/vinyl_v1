...
class ExecuteMixin(_compiler.SQLCompiler):
    ...
    def __Await__(self):
        self.results = self._execute_sql()
    ...
    def _execute_sql(self, result_type=MULTI):
        ...
        try:
            ...
            return iter([])
        return connections[self.using].execute_sql(sql, params)
...
class SQLInsertCompiler(DeferredCompilerMixin, _compiler.SQLInsertCompiler):
    ...
    def _execute_sql(self, returning_fields=None):
        assert not (
            returning_fields
